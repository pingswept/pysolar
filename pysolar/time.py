...
import warnings
import datetime
from .constants import SECONDSPERDAY
POSIXEPOCH = datetime.datetime(1970, 1, 1, tzinfo=datetime.timezone.utc)

def julian_century(jdn):
    """ number of julian days as century """
    return (jdn - 2451545.0) / 36525.0

JULIANDAYOFFSET = 1721425 - 0.5
"""
add to datetime.datetime.toordinal() to get Julian day number
"""

GREGORIANDAYOFFSET = 719163
"""
number of days to add to datetime.datetime.timestamp()
seconds_per_day to agree with datetime.datetime.toordinal()
"""
...
#-
LEAPSECONDSBASEYEAR = 1972
LEAPSECONDSAJUSTMENTS = \
    [ \
      (+1, +1), # 1972
      (0, +1), # 1973
      (0, +1), # 1974
...
def leap_seconds(when):
    ...
    while True:
        if year > when.tm_year:
            break
        if year - LEAPSECONDSBASEYEAR >= len(LEAPSECONDSAJUSTMENTS):
            if ( \
                    when.tm_year - LEAPSECONDSBASEYEAR > len(LEAPSECONDSAJUSTMENTS) \
                        or
                    when.tm_year - LEAPSECONDSBASEYEAR == len(LEAPSECONDSAJUSTMENTS)
                    and when.tm_mon > 6):
                warnings.warn \
                  ("I don't know about leap seconds after %d" % \
                        (LEAPSECONDSBASEYEAR + len(LEAPSECONDSAJUSTMENTS) - 1)
                  )
            #end if
            break
        #end if
        entry = LEAPSECONDSAJUSTMENTS[year - LEAPSECONDSBASEYEAR]
        if year == when.tm_year:
            if when.tm_mon > 6:
                adj += entry[0]
            #end if
...
def delta_t(when):
    ...
    if year < DELTATBASEYEAR:
        year = DELTATBASEYEAR
        month = 1
    elif year == DELTATBASEYEAR:
        month = max(0, month - DELTATBASEMONTH) + 1
    elif year >= DELTATBASEYEAR + len(DELTAT):
        year = DELTATBASEYEAR + len(DELTAT) - 1
    #end if
    if year == DELTATBASEYEAR + len(DELTAT) - 1:
        month = min(month, len(DELTAT[year - DELTATBASEYEAR]))
    #end if
    return \
        DELTAT[year - DELTATBASEYEAR][month - 1]
...
def timestamp(when):
    ...
    if when.tzinfo is None:
        ...
        return (when - POSIXEPOCH).total_seconds()

def julian_solar_day(when):
    ...
    return \
        ( \
            (timestamp(when) + leap_seconds(when) + TTOFFSET - delta_t(when))
            /
            SECONDSPERDAY
            +
            GREGORIANDAYOFFSET
...
#end julian_ephemeris_day

def julian_ephemeris_century(jed):
    """ number of julian days as a century """
    return (jed - 2451545.0) / 36525.0

def julian_ephemeris_millennium(jec):
    """ number of julian days as a millenium """
    return jec / 10.0
