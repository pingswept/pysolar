...
def extraterrestrial_irrad(when, latitude_deg, longitude_deg, vsc=SC_DEFAULT):
    """
    ...
    .. [2] Dr. J. Schumacher and et al,
           "INSEL LE(Integrated Simulation Environment Language)Block reference",p.68
    """
    day = when.utctimetuple().tm_yday
    vab = math.cos(2 * math.pi * (day - 1.0) / 365.0)
    vbc = math.sin(2 * math.pi * (day - 1.0) / 365.0)
    vcd = math.cos(2 * (2 * math.pi * (day - 1.0) / 365.0))
    vdf = math.sin(2 * (2 * math.pi * (day - 1.0) / 365.0))
    decl = solar.declination(day)
    vha = solar.hour_angle(when, longitude_deg)
    vza = math.sin(latitude_deg) * math.sin(decl) + \
      math.cos(latitude_deg) * math.cos(decl) * math.cos(vha)

    return vsc * vza * (1.00010 + 0.034221 * vab + 0.001280 * vbc + 0.000719 * vcd + 0.000077 * vdf)


def declination_degree(when, vty=TY_DEFAULT):
    ...
    """
    return constants.EARTH_AXIS_INCLINATION * \
      math.sin(2 * math.pi / vty * (when.utctimetuple().tm_yday - 81))

def solarelevation_function_clear(latitude_deg, longitude_deg, when,
   ...
   pressure : float
    ...
    """
    altitude = solar.altitude(latitude_deg, longitude_deg, when, elevation, temp, pressure)
    return 0.038175 + (1.5458 * math.sin(altitude)) + \
      -0.59980 * (0.5 * (1 - math.cos(2 * altitude)))
...
def solarelevation_function_overcast(latitude_deg, longitude_deg, when,
    ...
    """
    altitude = solar.altitude(latitude_deg, longitude_deg, when, elevation, temperature, pressure)
    return ((-0.0067133) + \
      (0.78600 * (math.sin(altitude)))) + (0.22401 * (0.5 * (1 - math.cos(2 * altitude))))


def diffuse_transmittance(TL=TL_DEFAULT):
    """Equation calculates the Diffuse_transmittance and the is the
       Theoretical Diffuse Irradiance on a horizontal
    ...
    References
    ----------
    .. [1] S. Younes, R.Claywell and el al,
           "Quality control of solar radiation data: present status and proposed
...
def diffuse_underclear(latitude_deg, longitude_deg, when, elevation=ELEVATION_DEFAULT,
                       temperature=constants.STANDARD_TEMPERATURE,
                       pressure=constants.STANDARD_PRESSURE, TL=TL_DEFAULT):
    ...
    latitude_deg : float
        latitude in decimal degree.
        A geographical term denoting the north/south angular location of a place on
        a sphere.
    longitude_deg : float
        longitude in decimal degree.
        Longitude shows your location in an east-west direction,relative to the
...
def diffuse_underovercast(latitude_deg, longitude_deg, when, elevation=ELEVATION_DEFAULT,
    ...
    return mean_earth_sun_distance(when) * DT * \
      solar.altitude(latitude_deg, longitude_deg, when, elevation,
...
def direct_underclear(latitude_deg, longitude_deg, when,
                      temperature=constants.STANDARD_TEMPERATURE,
                      pressure=constants.STANDARD_PRESSURE,
                      TY=TY_DEFAULT, AM=AM_DEFAULT, TL=TL_DEFAULT,
                      elevation=ELEVATION_DEFAULT):
    ...
    .. [1] S. Younes, R.Claywell and el al,
           ...
           new approaches", energy 30 (2005), pp 1533 - 1549.
    """
    MESD = mean_earth_sun_distance(when)

    DEC = declination_degree(when, TY)

    return 1367 * MESD * math.exp(-0.8662 * AM * TL * DEC) * \
      math.sin(solar.altitude(latitude_deg, longitude_deg, when,
...
def global_irradiance_clear(dirc, diffc, latitude_deg, longitude_deg, when,
    ...
    AM : float
        ...
        One air mass, or AM1, is the thickness of the Earth's atmosphere.
        Air mass zero (AM0) describes solar irradiance in
        space, where it is unaffected by the atmosphere.
...
def global_irradiance_overcast(latitude_deg, longitude_deg, when,
    ...
    """
    return 572 * solar.altitude(latitude_deg, longitude_deg, when,
                                elevation, temperature, pressure)
...
def clear_index(ghi_data, when, latitude_deg, longitude_deg):
    ...
    """
    return ghi_data / extraterrestrial_irrad(when, latitude_deg, longitude_deg)
